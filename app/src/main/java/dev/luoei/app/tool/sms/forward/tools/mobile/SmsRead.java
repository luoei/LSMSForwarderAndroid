...
public class SmsRead {
     ...
     * @return 所有短信
     */
    public List<Map<String,String>> getSmsFromPhone() {
        Cursor cursor = CommonParas.getMainContext().getContentResolver().query(
...
//            }
           ...
           // String name = cursor.getString(cursor.getColumnIndex("person"));//联系人姓名列表
            ...
            map.put(CommonVariables.SMS_PHONE,number);
            map.put(CommonVariables.SMS_BODY,body);
            map.put(CommonVariables.SMS_SEND_DATE,sDateTime);
            list.add(map);
